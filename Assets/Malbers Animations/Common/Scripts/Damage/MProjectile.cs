...
namespace MalbersAnimations.Weapons
{
    ...
    public class MProjectile : MDamager, IProjectile
    {
        ...
        protected Transform cachedTransform; //CustomPatch: added cached transform (TODO: should be added in abstract class in editor OnValidate() and Reset() calls to auto-set reference to a serialized readonly inspector property (not in Awake() or Start() calls)
        ...
        {
            //CustomPatch: added cached transform
        ...
        public virtual void Pool_Release(GameObject gameObject) => Pool.Release(gameObject);
        ...
        protected virtual void OnCollisionEnter(UnityEngine.Collision other)
        {
            ...
            if (Prev_pos == Vector3.zero) Prev_pos = cachedTransform.position;
        ...
        public virtual void ProjectileImpact(Rigidbody targetRB, Collider collider, Vector3 hitPosition, Vector3 normal) //CustomPatch: correctly named param HitPosition to lower-case (highly recommended to name method params and local variables lower-case => class properties use this naming convention and you can easily generate bugs by using one over the other)
        {
            ...
            try
            {
                ...
                switch (impactBehaviour)
                {
                    ...
                    case ImpactBehaviour.DisableOnImpact:
                        ...
                        //CustomPatch: removed redundant empty default switch statement
                }
            ...
            {
                //CustomPatch: moved hit events at the end of the execution of this method to make sure they're called with the latest up-to-date post-processed impact information (penetration adjustment, projectile re-parenting, hit position adjustments, etc)
                if (!attackMissed)
                {
        ...
        protected virtual void Stick_On_Surface(Transform colliderTransform, Vector3 HitPosition) //CustomPatch: renamed the previous "collider" param to avoid confusion (good practices creates an easier to maintain code base ;) )
        {
            ...
            cachedTransform.position += NormalizedDirection * Penetration; //Put the Projectile a bit deeper in the collider
