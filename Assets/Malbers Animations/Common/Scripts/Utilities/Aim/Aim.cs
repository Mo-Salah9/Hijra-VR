...
#pragma warning restore CS0414 // Remove unused private members
        ...
        public virtual void AimLogic(bool useRaycasting)
        {
            ...
            if (LastAimHit.transform != AimHit.transform)
            {
        ...
        protected virtual RaycastHit GetAimAssist(RaycastHit hit)
        {
            ...
            if (LastCollider != hitCollider) //Only check for AimAssist when the collider is different
            {
                ...
                if (!Assist.IsUnityRefNull()) //CustomPatch!: corrected null check for unity object interface type
                {
                    if (Assist.AimAssist)
                    {
                        IsTargetAssist = true;
                    ...
                    {
                        if (!LastAimTarget.IsUnityRefNull()) //corrected null check for unity object interface type
