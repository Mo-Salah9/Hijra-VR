...
namespace MalbersAnimations.Controller.AI
{
    ...
    public class MAnimalAIControl : MonoBehaviour, IAIControl, IAnimatorListener
    {
        ...
        public virtual void Updating()
        {
            ...
            {
                //if (animal.ActiveStateID == StateEnum.Jump)
                //{
                //    //Debug.Log("JUMP");
                //    //animal.Move(AIDirection);

                //    Debug.DrawRay(animal.transform.position, AIDirection * 2, Color.red, 0.5f);

                //}
                return;    //Do nothing while is in an offmeshLink or its Waiting
            }
            CheckMovingTarget();
        ...
        {
            //CustomPatch: BUGGED: TODO: needs refactor => when the animal movement is stopped the agent should NOT be disabled => there will be no local avoidance with other agents that aren't stopped
            ActiveAgent = false; //Disable the Agent 
        ...
        protected virtual void Update_DestinationPosition()
        {
            if (UpdateDestinationPosition)
            {
                ...
                //CustomPatch: Added auto validate destination position logic
                if (!autoValidateDestination && TargetTooHigh && StopOnTargetTooHigh)
                {
                    Stop();
                    return;
                }
        ...
        public bool TargeterOn => !ITargeter.IsUnityRefNull() && ITargeter.Targeters > 0; //CustomPatch: corrected null check of unity object interface type
        ...
        public virtual void SetTarget(Transform newTarget, bool move)
        {
            ...
            if (!ITargeter.IsUnityRefNull() && newTarget != target) //CustomPatch: corrected null check of unity object interface type
            {
            ...
            if (target != null)
            {
                ...
                if (!IsAITarget.IsUnityRefNull() && IsAITarget is IAITargeterTarget targ) //CustomPatch: corrected null check of unity object interface type
                {
                    ITargeter = targ;
                ...
                Debuging($"<color=yellow>New Target <B>[{newTarget.name}]</B> → [{DestinationPosition}]. Move = [{move}] IsAiTarget {!IsAITarget.IsUnityRefNull()}</color>"); //CustomPatch: corrected null check of unity object interface type
        ...
        {
            var TargetPos = (!IsAITarget.IsUnityRefNull()) ? IsAITarget.GetCenterPosition(Index) : target.position; //CustomPatch: corrected null check of unity object interface type
        ...
        public virtual float GetTargetStoppingDistance() => !IsAITarget.IsUnityRefNull() ? IsAITarget.StopDistance() : stoppingDistance * animal.ScaleFactor; //CustomPatch: corrected null check of unity object interface type
        public virtual float GetTargetSlowingDistance() => !IsAITarget.IsUnityRefNull() ? IsAITarget.SlowDistance() : slowingDistance * animal.ScaleFactor; //CustomPatch: corrected null check of unity object interface type
        ...
        protected virtual void OnDrawGizmos()
        {
            ...
            if (debugGizmos)
            {
                ...
                if (AgentTransform)
                {
                    ...
                    if (!ITargeter.IsUnityRefNull() && Index != -1) //CustomPatch: corrected null check of unity object interface type
                    {
